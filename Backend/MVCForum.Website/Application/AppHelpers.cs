...
namespace MVCForum.Website.Application
{
    public static class AppHelpers
    {
        ...
        public static string DoctorImage(string avatar,string path, int width,int height)
        {
            if (!string.IsNullOrEmpty(avatar))
            {
                // Has an avatar image
             
                return string.Format("/Content/uploads/ImageDoctors/{0}?width={1}&crop=0,0,{1},{2}", avatar,width,height);
            }

            return "/Content/assets/images/default-ava.svg"; // Hình default
        }
        ...
        public static UploadFileResult UploadFile(HttpPostedFileBase file, string uploadFolderPath, ILocalizationService localizationService, bool onlyImages = false)
        {
            ...
            var fileUploadAllowed = "jpg,jpeg,png,gif,pdf,doc,docx,xls,xlsx";
            ...
            if (fileName != null)
            {
                ...
                var allowedFileExtensions = fileUploadAllowed;
        ...
        public static UploadFileResult UploadRecruitmentFile(HttpPostedFileBase file, string uploadFolderPath, ILocalizationService localizationService, bool onlyImages = false)
        {
            var upResult = new UploadFileResult { UploadSuccessful = true };
            const string imageExtensions = "jpg";
            var fileUploadAllowed = "doc,docx";
            var fileName = Path.GetFileName(file.FileName);
            var storageProvider = StorageProvider.Current;
            if (fileName != null)
            {
                ...
                {
                    allowedFileExtensions = imageExtensions;
                    if (file.ContentLength > Convert.ToInt32(SiteConstants.Instance.ImageUploadMaximumFileSizeInBytes))
                    {
                        //File is too big
                        //forum.config :  <setting key="ImageUploadMaximumFileSizeInBytes" value="2097152" />
                        upResult.UploadSuccessful = false;
                        upResult.ErrorMessage = localizationService.GetResourceString("Post.UploadFileTooBig");
                        return upResult;
                    }
        ...
        public static string UrlImage(string url,string size)
        {
            var newUrl = url.Split('/').ToList();
            var filename = newUrl.Last();
            var newlist = newUrl.Remove(filename);
            return url.Replace(filename, string.Format("{0}/{1}", size, url));
        }
        public static string TopicImage(string url,  Guid userId, int width,int height)
        {
            if (!string.IsNullOrEmpty(url))
            {
                // Has an avatar image
                var storageProvider = StorageProvider.Current;
                return storageProvider.BuildFileUrl(userId, "/", url, string.Format("?width={0}&crop=0,0,{0},{1}", width,height));
            }

            return "/Content/Images/default.jpg";
        }
        public static string DoctorImage(string url, int width, int height)
        {
            if (!string.IsNullOrEmpty(url))
            {
                // Has an avatar image
                var storageProvider = StorageProvider.Current;
                return storageProvider.BuildFileUrl("/ImageDoctors", "/", url, string.Format("?width={0}&crop=0,0,{0},{1}", width, height));
            }
        ...
        {
             var data = StringUtils.RemoveSign4VietnameseString(str).Replace(" ", "").ToLower();
            return data;
