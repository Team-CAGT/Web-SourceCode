...
namespace MVCForum.Website.Controllers
{
    public partial class TopicController : BaseController
    {
        ...
        public TopicController(ILoggingService loggingService, IUnitOfWorkManager unitOfWorkManager, IMembershipService membershipService, IRoleService roleService, ITopicService topicService, IPostService postService,
            ...
            ICacheService cacheService, ITagNotificationService tagNotificationService, IPostEditService postEditService, IContactService contactService)
                ...
                if (post.User.Id == LoggedOnReadOnlyUser.Id || permissions[SiteConstants.Instance.PermissionEditPosts].IsTicked)
                {
                    ...
                    if (allowedAccessCategories.Any() && LoggedOnReadOnlyUser.DisablePosting != true)
                    {
                        ...
                        var viewModel = new CreateEditTopicViewModel
                        {
                            ...
                            MetaSeo = topic.MetaSeo,
                            TypeTopic = topic.TypeTopic,
                            LinkVideo = topic.LinkVideo,
                            LinkImage = topic.LinkImage,
                            IsTopicStarter = true,
                            Files = new HttpPostedFileBase[2]
                        };
                ...
                if (ModelState.IsValid)
                {
                    ...
                    try
                    {
                        ...
                        if (post.User.Id == LoggedOnReadOnlyUser.Id || permissions[SiteConstants.Instance.PermissionEditPosts].IsTicked)
                        {
                            ...
                            if (editPostViewModel.Files != null)
                            {
                                    ...
                                    LoggedOnReadOnlyUser.DisableFileUploads != true)
                                {
                                    ...
                                    foreach (var file in editPostViewModel.Files)
                                    {
                                        if (file != null)
                                        {
                                            ...
                                            var linkhinhanh = uploadResult.UploadedFileName;
                                            if (i == 0)
                                            {
                                                topic.LinkImage = linkhinhanh;
                            ...
                            if (post.IsTopicStarter)
                            {
                                ...
                                int typetopic = 0;
                                int.TryParse(Request.Form["rdTypeTopic"], out typetopic);
                                topic.TypeTopic = typetopic;
                                ...
                                topic.Description = editPostViewModel.Description;
                            //    topic.Pending = false;
                                // Update post edit
                ...
                using (var unitOfWork = UnitOfWorkManager.NewUnitOfWork())
                {
                    ...
                    else
                    {
                        ...
                        topic = new Topic
                        {
                            ...
                            MetaSeo = topicViewModel.MetaSeo,
                            TypeTopic = typetopic,
                            LinkVideo = topicViewModel.LinkVideo,
                            Pending = true
                        };
                ...
                using (var unitOfWork = UnitOfWorkManager.NewUnitOfWork())
                {
                    if (successfullyCreated && !cancelledByEvent)
                    {
                        ...
                        // Redirect to the newly created topic
                         return Redirect($"{topic.NiceUrlReview}?postbadges=true");
        ...
        public ActionResult LatestTopics(int? p)
        {
            using (UnitOfWorkManager.NewUnitOfWork())
            {
                var allowedCategories = _categoryService.GetAllowedCategories(UsersRole).Where(x => x.Slug == "y-hoc" || x.Slug == "su-kien").ToList();
                var settings = SettingsService.GetSettings();
        ...
        [ChildActionOnly]
        public ActionResult Goctrian(int? p)
        {
            using (UnitOfWorkManager.NewUnitOfWork())
            {
                var allowedCategories = _categoryService.GetAllowedCategories(UsersRole).Where(x => x.Slug == "goc-tri-an").ToList();
                var settings = SettingsService.GetSettings();
        ...
        #region Partial Common

        [ChildActionOnly]
        public ActionResult Video(int? amountToShow)
        {
            using (UnitOfWorkManager.NewUnitOfWork())
            {
                if (amountToShow == null)
                ...
                var cacheKey = string.Concat("Video", UsersRole.Id, amountToShow);
                var viewModel = _cacheService.Get<HotTopicsViewModel>(cacheKey);
                if (viewModel == null)
                {
                    // Allowed Categories
                    ...
                    // Get the topics
                    var topics = _topicService.GetRecentTopicsbyType(allowedCategories, 2, (int)amountToShow);
                    ...
                    _cacheService.Set(cacheKey, viewModel, CacheTimes.OneMinute);
        ...
        [ChildActionOnly]
        public ActionResult Baivietnoibac(DateTime? from, DateTime? to, int? amountToShow)
        {
            ...
            using (UnitOfWorkManager.NewUnitOfWork())
            {
                ...
                var cacheKey = string.Concat("Baivietnoibac", UsersRole.Id, fromString, toString, amountToShow);
                var viewModel = _cacheService.Get<HotTopicsViewModel>(cacheKey);
                if (viewModel == null)
                {
                    // Allowed Categories
        ...
        public ActionResult Images(int? amountToShow)
        {
            ...
            using (UnitOfWorkManager.NewUnitOfWork())
            {
                ...
                if (viewModel == null)
                {
                    ...
                    // Get the topics
                    var topics = _topicService.GetRecentTopicsbyType(allowedCategories, 1, (int)amountToShow);
        ...
        [ChildActionOnly]
        public ActionResult TopService(DateTime? from, DateTime? to, int? amountToShow)
        {
            ...
            using (UnitOfWorkManager.NewUnitOfWork())
            {
                ...
                var cacheKey = string.Concat("TopService", UsersRole.Id, amountToShow);
                var viewModel = _cacheService.Get<HotTopicsViewModel>(cacheKey);
                if (viewModel == null)
                {
                    // Allowed Categories
                    var allowedCategories = _categoryService.GetAllowedCategories(UsersRole).Where(x => x.Slug.Equals("dich-vu")).ToList();

                    // Get the topics
                    var topics = _topicService.GetRecentTopics(1, (int)amountToShow, (int)amountToShow, allowedCategories);
        ...
        [ChildActionOnly]
        public ActionResult Truyenthong(int? p)
        {

            using (UnitOfWorkManager.NewUnitOfWork())
            {
                var allowedCategories = _categoryService.GetAllowedCategories(UsersRole).Where(x => x.Slug == "truyen-thong").ToList();
                var settings = SettingsService.GetSettings();
        ...
        #endregion
        public ActionResult Article(string slug, int? p)
        {
            if (string.IsNullOrEmpty(slug))
            {
                slug = "10-su-that-thu-vi-ve-bo-nao";
            }
            // Set the page index
            var pageIndex = p ?? 1;
        ...
        public ActionResult Review(string slug, int? p)
        {
            if (string.IsNullOrEmpty(slug))
            {
                slug = "10-su-that-thu-vi-ve-bo-nao";
            ...
            {
                // Get the topic
                var topic = _topicService.GetTopicIsPendingBySlug(slug);
        ...
        [ChildActionOnly]
        public ActionResult InternalNews(DateTime? from, DateTime? to, int? amountToShow)
        {

            return PartialView();
        }
        [ChildActionOnly]
        public ActionResult NewsTopics(int? p)
        {
            using (UnitOfWorkManager.NewUnitOfWork())
            {
                var allowedCategories = _categoryService.GetAllowedCategories(UsersRole).Where(x => x.Slug == "y-hoc" || x.Slug == "su-kien").ToList();
        ...
        [ChildActionOnly]
        public ActionResult Hoatdongnoibat(int? amountToShow)
        {
            ...
            using (UnitOfWorkManager.NewUnitOfWork())
            {
                ...
                var cacheKey = string.Concat("Hoatdongnoibat", UsersRole.Id, amountToShow);
                var viewModel = _cacheService.Get<HotTopicsViewModel>(cacheKey);
                if (viewModel == null)
                {
                    // Allowed Categories
                    var allowedCategories = _categoryService.GetAllowedCategories(UsersRole).Where(x => x.Slug.Equals("hoat-đong-noi-bat")).ToList();
        ...
        public ActionResult Lichkham(string slug, int? p)
        {
            if (string.IsNullOrEmpty(slug))
            {
                slug = "danh-sach-bac-si-tai-khoa-kham-benh";
            }
            // Set the page index
            var pageIndex = p ?? 1;
        ...
        public ActionResult ArticleQuanlyChatluong(string slug, int? p)
        {
            if (string.IsNullOrEmpty(slug))
            {
                slug = "kham-benh-phat-thuoc-va-tang-qua-cho-nguoi-dan-ben-tre";
            }
            // Set the page index
            var pageIndex = p ?? 1;
        ...
        public ActionResult LichCongtac(string slug, int? p)
        {
            if (string.IsNullOrEmpty(slug))
            {
                slug = "lich-cong-tac";
        ...
        #region Rss
        [ChildActionOnly]
        public ActionResult RssVanban()
        {
            // var test = RSSReader.
            // var vm = RSSReader.GetRSSFeed("http://medinet.gov.vn/rssbenhvien.aspx?kt=vb");
            var cacheKey = string.Concat("RssVanban");
            var vm = _cacheService.Get<RssViewModel>(cacheKey);

            if (vm == null)
            {
                vm = new RssViewModel();
                vm.Vanbans = RSSReader.GetRSSFeed("http://medinet.gov.vn/rssbenhvien.aspx?kt=vb")
                    .ToList().OrderByDescending(x => x.Date).ToList();
                vm.Thongbaos = RSSReader.GetRSSFeed("http://medinet.gov.vn/rssbenhvien.aspx?kt=tb").ToList().OrderByDescending(x => x.Date)
                    .ToList();
                vm.Thumois = RSSReader.GetRSSFeed("http://medinet.gov.vn/rssbenhvien.aspx?kt=tm").ToList().OrderByDescending(x => x.Date)
                    .ToList();
                vm.LichCogtacs = RSSReader.GetRSSFeed("http://medinet.gov.vn/rssbenhvien.aspx?kt=tt").ToList().OrderByDescending(x => x.Date)
                    .ToList();
                //vm.Tintucs = RSSReader.GetRSSFeed("http://medinet.gov.vn/rssbenhvien.aspx?kt=th").ToList().OrderByDescending(x => x.Date)
                //    .ToList();
                if (vm.Vanbans.Count() == 0)
                {
                    // Allowed Categories
                    var rs = _contactService.GetListInfoFromSoyte();

                    foreach (var item in rs)
                    {

                        var rss = new RSS() { Title = item.Title, Link = item.Url, PubDate = item.NgayApdung.ToString("dd/MM/yyyy") };
                        TimeSpan check = DateTime.Now - item.NgayApdung;
                        ...
                        {
                            vm.Thongbaos.Add(rss);
                        }

                    }
                }
               
                _cacheService.Set(cacheKey, vm, CacheTimes.OneHour);
        ...
        public ActionResult XemthemRssVanban()
        {
            var cacheKey = string.Concat("RssVanban");
            var vm = _cacheService.Get<RssViewModel>(cacheKey);
            if (vm == null)
            {
                vm = new RssViewModel();
                vm.Vanbans = RSSReader.GetRSSFeed("http://medinet.gov.vn/rssbenhvien.aspx?kt=vb").ToList();
                if (vm.Vanbans == null || vm.Vanbans.Count() == 0)
                {
                    var rs = _contactService.GetListInfoFromSoytebyKey("VANBAN");
                    foreach (var item in rs)
                    {
                        var rss = new RSS() { Title = item.Title, Link = item.Url, PubDate = item.NgayApdung.ToString("dd/MM/yyyy") };
                        vm.Vanbans.Add(rss);
                ...
                _cacheService.Set(cacheKey, vm, CacheTimes.FiveMinute);
        ...
        [ChildActionOnly]
        [OutputCache(Duration = (int)CacheTimes.TwoHours)]
        public ActionResult XemthemRssThongbao()
        {
            // var test = RSSReader.
            // var vm = RSSReader.GetRSSFeed("http://medinet.gov.vn/rsscdtc.aspx?kt=tm");
            var cacheKey = string.Concat("RssVanban");
            var vm = _cacheService.Get<RssViewModel>(cacheKey);
            if (vm == null)
            {
                vm = new RssViewModel();
                vm.Thongbaos = RSSReader.GetRSSFeed("http://medinet.gov.vn/rssbenhvien.aspx?kt=tb").ToList();
                if (vm.Thongbaos == null || vm.Thongbaos.Count() == 0)
                {
                    // Allowed Categories
                    var rs = _contactService.GetListInfoFromSoytebyKey("THONGBAO");
                    foreach (var item in rs)
                    {
                        var rss = new RSS() { Title = item.Title, Link = item.Url, PubDate = item.NgayApdung.ToString("dd/MM/yyyy") };
                        vm.Thongbaos.Add(rss);
        ...
        [ChildActionOnly]
        [OutputCache(Duration = (int)CacheTimes.TwoHours)]
        public ActionResult XemthemRssThumoi()
        {
            // var test = RSSReader.
            // var vm = RSSReader.GetRSSFeed("http://medinet.gov.vn/rsscdtc.aspx?kt=tm");
            var cacheKey = string.Concat("RssVanban");
            ...
            {
                vm = new RssViewModel();
                vm.Thumois = RSSReader.GetRSSFeed("http://medinet.gov.vn/rssbenhvien.aspx?kt=tm").ToList();
                if (vm.Thumois == null || vm.Thumois.Count() == 0)
                {
                    // Allowed Categories
                    var rs = _contactService.GetListInfoFromSoytebyKey("THUMOI");
                    foreach (var item in rs)
                    {
                        var rss = new RSS() { Title = item.Title, Link = item.Url, PubDate = item.NgayApdung.ToString("dd/MM/yyyy") };
                        vm.Thumois.Add(rss);
        ...
        [ChildActionOnly]
        [OutputCache(Duration = (int)CacheTimes.TwoHours)]
        public ActionResult XemthemRssTintuc()
        {
            // var test = RSSReader.
            // var vm = RSSReader.GetRSSFeed("http://medinet.gov.vn/rsscdtc.aspx?kt=tm");
            var cacheKey = string.Concat("RssVanban");
            ...
            {
                vm = new RssViewModel();
                vm.Tintucs = RSSReader.GetRSSFeed("http://medinet.gov.vn/rssbenhvien.aspx?kt=th").ToList();
                if (vm.Thumois == null || vm.Thumois.Count() == 0)
                {
                    // Allowed Categories
                    var rs = _contactService.GetListInfoFromSoytebyKey("THUMOI");
        ...
        [ChildActionOnly]
        [OutputCache(Duration = (int)CacheTimes.TwoHours)]
        public ActionResult XemthemRssLichContac()
        {
            // var test = RSSReader.
            // var vm = RSSReader.GetRSSFeed("http://medinet.gov.vn/rsscdtc.aspx?kt=tm");
            var cacheKey = string.Concat("RssVanban");
            ...
            {
                vm = new RssViewModel();
                vm.LichCogtacs = RSSReader.GetRSSFeed("http://medinet.gov.vn/rssbenhvien.aspx?kt=tt").ToList();
                if (vm.LichCogtacs == null || vm.LichCogtacs.Count() == 0)
                {
                    // Allowed Categories
                    var rs = _contactService.GetListInfoFromSoytebyKey("LICHCONGTAC");
                    foreach (var item in rs)
                    {
                        var rss = new RSS() { Title = item.Title, Link = item.Url, PubDate = item.NgayApdung.ToString("dd/MM/yyyy") };
                        vm.Tintucs.Add(rss);
        ...
        //duyệt topic
        [Authorize(Roles = "Admin,VietBai")]
        public ActionResult ChangePendingTopic(Guid id)
        {
            using (var unitOfWork = UnitOfWorkManager.NewUnitOfWork())
            {
                // Get the topic
                ...
                unitOfWork.Commit();
                return Redirect($"{topic.NiceUrl}?postbadges=true");
                //return RedirectToAction("Article",topic.Slug,1);
            }
        }
