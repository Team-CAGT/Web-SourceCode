    ...
    public partial class TopicService : ITopicService
    {
        ...
        public Topic GetTopicBySlug(string slug)
        {
            ...
            return _context.Topic
                    ...
                    .Include(x => x.User)
                    .Include(x => x.Poll)
                    .FirstOrDefault(x => x.Slug == slug && (x.Pending != null && x.Pending.Value == false));
        }
        /// <summary>
        /// Return a topic by url slug
        /// </summary>
        /// <param name="slug"></param>
        /// <returns></returns>
        public Topic GetTopicIsPendingBySlug(string slug)
        {
            slug = StringUtils.GetSafeHtml(slug);
            return _context.Topic
                .Include(x => x.Category)
                ...
                .Include(x => x.User)
                .Include(x => x.Poll)
                .FirstOrDefault(x => x.Slug == slug && (x.Pending != null && x.Pending.Value));
        }
        ...
        public PagedList<Topic> GetPagedTopicsAll(int pageIndex, int pageSize, List<Category> allowedCategories)
        {
            // get the category ids
            var allowedCatIds = allowedCategories.Select(x => x.Id);
            ...
            var total = _context.Topic
                .AsNoTracking()
                .Count(x => allowedCatIds.Contains(x.Category.Id));
            // Get the topics using an efficient
            var results = _context.Topic
                    .AsNoTracking()
                    .Where(x => allowedCatIds.Contains(x.Category.Id))
        ...
        public IList<Topic> GetRecentTopicsbyType(List<Category> allowedCategories, int type, int amountToShow = 20)
        {
            ...
            var topics = _context.Topic
                ...
                .Include(x => x.Posts)
                .Include(x => x.User)
                .Where(x => allowedCatIds.Contains(x.Category.Id) && x.TypeTopic == type)
                .OrderByDescending(x => x.CreateDate)
                .Take(amountToShow)
                .ToList();
        ...
        /// <summary>
        /// Update pending topic
        /// </summary>
        /// <param name="topic"></param>
        public void UpdatePendingTopic(Topic topic)
        {
        ...
        public PagedList<Topic> GetAllPendingTopics(int pageIndex, int pageSize, List<Category> allowedCategories)
        {
            // get the category ids
            var allowedCatIds = allowedCategories.Select(x => x.Id);
        ...
        public PagedList<Topic> SearchAllTopics(string search, int pageIndex, int pageSize, List<Category> allowedCategories)
        {
            var slug = ServiceHelpers.CreateUrl(search);
            ...
            var total = _context.Topic
                .Count(x => x.Slug.Contains(slug) && allowedCatIds.Contains(x.Category.Id));

            // Get the topics using an efficient
            var results = _context.Topic
                .Where(x => x.Slug.Contains(slug) && allowedCatIds.Contains(x.Category.Id))
                .OrderBy(x => x.Name)
                .Skip((pageIndex - 1) * pageSize)
                .Take(pageSize)
                .ToList();
        ...
        public PagedList<Topic> GetPagedActiveTopicsAll(int pageIndex, int pageSize, List<Category> allowedCategories)
        {
            // get the category ids
            var allowedCatIds = allowedCategories.Select(x => x.Id);
            var total = _context.Topic
        ...
        public PagedList<Topic> SearchAllActiveTopics(string search, int pageIndex, int pageSize, List<Category> allowedCategories)
        {
            var slug = ServiceHelpers.CreateUrl(search);
