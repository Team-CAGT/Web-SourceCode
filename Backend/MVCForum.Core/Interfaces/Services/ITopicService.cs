...
namespace MVCForum.Domain.Interfaces.Services
{
    public partial interface ITopicService
    {
        ...
        PagedList<Topic> GetPagedTopicsAll(int pageIndex, int pageSize, List<Category> allowedCategories);
        IList<Topic> GetTopicBySlugLike(string slug);
        IList<Topic> GetRecentTopicsbyType( List<Category> allowedCategories,int type, int amountToShow = 20);


        void UpdatePendingTopic(Topic topic);
        PagedList<Topic> GetAllPendingTopics(int pageIndex, int pageSize, List<Category> allowedCategories);
        PagedList<Topic> SearchPendingTopics(string search, int pageIndex, int pageSize, List<Category> allowedCategories);
        Topic GetTopicIsPendingBySlug(string slug);
        PagedList<Topic> SearchAllTopics(string search, int pageIndex, int pageSize, List<Category> allowedCategories);
        PagedList<Topic> GetPagedActiveTopicsAll(int pageIndex, int pageSize, List<Category> allowedCategories);
        PagedList<Topic> SearchAllActiveTopics(string search, int pageIndex, int pageSize, List<Category> allowedCategories);
    }
}
